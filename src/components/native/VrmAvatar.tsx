...
const VrmAvatar = forwardRef(
  (
    ...
    // works everywhere, but VRMLoaderPlugin implements wrong types
    // difference between three-std-lib and three/examples/jsm/loaders/GLTFLoader ???
    const gltfDrei = useGLTF(vrmUrl, false, false, (loader) =>
      loader.register(
        (parser) => new VRMLoaderPlugin(parser, { autoUpdateHumanBones: true })
      )
    );

    console.log(gltfDrei);
